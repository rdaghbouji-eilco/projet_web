...
if (!empty($data->email) && !empty($data->password)) {
    ...
    if ($user && password_verify($data->password, $user['password'])) {
        // Connexion réussie, enregistrer l'ID utilisateur et le rôle dans la session
        $_SESSION['user'] = [
            'ID' => $user['ID'],
            'email' => $user['email'],
            'role' => $user['role'], // Ajouter le rôle à la session
            'name' => $user['name'],       // Ajoute le nom
            'surname' => $user['surname'] // Ajoute le prénom
        ];
