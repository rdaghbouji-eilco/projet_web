...
async function loadOffers() {
    try {
        ...
        const response = await fetch(fullUrl); // Fetch offers from the API
        if (!response.ok) {
...
async function editOffer(offerId) {
    try {
        ...
        const response = await fetch(`${fullUrl}?id=${offerId}`);
        if (!response.ok) {
...
async function populateDropdown(apiUrl, dropdownId, dataKey, defaultOption) {
    try {
        ...
        console.log(`Fetching data from: ${fullUrl}`);
        const response = await fetch(fillUrl); // Fetch data from the API
        if (!response.ok) {
